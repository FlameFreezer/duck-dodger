class PlayerBullet extends Phaser.GameObjects.Sprite {
    ...
    update(delta, homingLevel) {
        ...
        this.angle = 90 + direction * Math.acos(vecDot(vecNormalize(this.velocity), {x: 0, y: -1})) * RAD_TO_DEG;
    ...
    doHoming(delta, homingLevel) {
        ...
        for(let duck of this.scene.waveController.ducks) {
            if(duck.active && duck.hittable && Phaser.Geom.Intersects.CircleToRectangle(duck.hitbox, this.homingRegion)) {
